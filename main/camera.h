...
namespace camera
{
	...
	struct Info
	{
        ...
        Info() : posV{}, posVDest{}, posR{}, posRDest{}, posU{}, rot{}, mtxProjection{}, mtxView{}, fDistance(0.0f), fDistanceMin(0.0f), fDistanceMax(0.0f) {}
        Info(const D3DXVECTOR3& posV, const D3DXVECTOR3& posR, const D3DXVECTOR3& posU, const D3DXVECTOR3& rot, float fDistance) : posV(posV), posVDest(posV), posR(posR), posRDest(posR), posU(posU), rot(rot), mtxProjection{}, mtxView{}, fDistance(fDistance), fDistanceMin(CAMERA_DISTANCE_MIN), fDistanceMax(CAMERA_DISTANCE_MAX) {}
        Info(const D3DXVECTOR3& posV, const D3DXVECTOR3& posR, const D3DXVECTOR3& posU, const D3DXVECTOR3& rot, float fDistance, float fDistanceMin, float fDistanceMax) : posV(posV), posVDest(posV), posR(posR), posRDest(posR), posU(posU), rot(rot), mtxProjection{}, mtxView{}, fDistance(fDistance), fDistanceMin(fDistanceMin), fDistanceMax(fDistanceMax) {}
        ~Info() = default;
	};
}
...
public:
	...
	void Set(Index idx, const context::DrawContext drawContext); // 描画処理
