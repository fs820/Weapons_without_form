...
//初期化処理
//------------------------------
bool CRenderer::Init(const context::InitContext initContext)
{
	//オブジェクト生成
	...
	//ディスプレイモード
	DisplayMode displayMode{};// ディスプレイモードの変数宣言
	...
	// クライアント領域のサイズを取得
	RECT clientRect = {};
	if (FAILED(initContext.pAppContext->pWindow->GetClientRect(&clientRect))) return E_FAIL; // クライアントサイズの取得

	//バックバッファの設定
	m_d3dpp.hDeviceWindow = initContext.pAppContext->pWindow->GethWnd();
	...
	m_d3dpp.Windowed = !initContext.pAppContext->pWindow->IsFullScreen();
...
bool CRenderer::Draw(const context::DrawContext drawContext)
{
	...
	if (FAILED(m_pD3DDevice->Clear(0, nullptr, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL, D3DCOLOR_RGBA(0, 0, 0, 0), 1.0f, 0))) { return E_FAIL; }
	...
	if (SUCCEEDED(m_pD3DDevice->BeginScene()))//描画開始
	{
        ...
        CObject::DrawPriority(3, drawContext, true); // 残像オブジェクトの描画
	...
	if (SUCCEEDED(m_pD3DDevice->BeginScene()))//描画開始
	{
		...
		CObject::DrawPriority(0, drawContext); // オブジェクトの描画
		CObject::DrawPriority(1, drawContext); // オブジェクトの描画
		m_pD3DDevice->EndScene(); // 描画終了
	}
	else
	{
	...
	if (SUCCEEDED(m_pD3DDevice->BeginScene()))//描画開始
	{
		...
		CObject::DrawPriority(3, drawContext); // オブジェクトの描画
		CObject::DrawPriority(4, drawContext); // オブジェクトの描画
		m_pD3DDevice->EndScene(); // 描画終了
	}
	else
	{
	...
	if (SUCCEEDED(m_pD3DDevice->BeginScene()))//描画開始
	{
		...
		CManager::GetCamera()->Set(0);
		CObject::DrawPriority(5, drawContext); // オブジェクトの描画
		CObject::DrawPriority(6, drawContext); // オブジェクトの描画
		m_pD3DDevice->EndScene(); // 描画終了
	}
	else
	{
	...
	{
		SetRender(); // 描画設定
		CObject::DrawPriority(7, drawContext);  // オブジェクトの描画
		m_pD3DDevice->EndScene();  // 描画終了
	}
	else
	{
	...
	if (SUCCEEDED(m_pD3DDevice->BeginScene()))//描画開始
	{
		...
		drawContext.pAppContext->pDebug->Draw();        // デバック表示
		m_pD3DDevice->EndScene();  // 描画終了
	}
	else
	{
...
// 頂点設定
//------------------------------
bool CRenderer::SetVertex2D(LPDIRECT3DVERTEXBUFFER9* ppVtxBuffer, span<const DirectX2D::Vertex> vertex, DirectX::Vector3 offSet, float angle) const
{
	void* pVtx{};
...
// 頂点設定
//------------------------------
bool CRenderer::SetVertex2D(LPDIRECT3DVERTEXBUFFER9* ppVtxBuffer, DirectX::Vector2 size, DirectX::Vector3 pos, float angle, DirectX::Color color, DirectX::Vector2 texBlock) const
{
...
void CRenderer::SetRender(void) const
{
	...
	m_pD3DDevice->SetRenderState(D3DRS_STENCILENABLE, FALSE);            // ステンシルバッファを無効化
	m_pD3DDevice->SetRenderState(D3DRS_COLORWRITEENABLE, 0x0000000F);    // カラーバッファ書き込みを有効化
	m_pD3DDevice->SetRenderState(D3DRS_STENCILFUNC, D3DCMP_ALWAYS);      // ステンシル関数を常に通過
	m_pD3DDevice->SetRenderState(D3DRS_STENCILPASS, D3DSTENCILOP_ZERO);  // ステンシルパスをゼロに設定
	m_pD3DDevice->SetRenderState(D3DRS_STENCILZFAIL, D3DSTENCILOP_ZERO); // ステンシルZフェイルをインクリメントに設定
	m_pD3DDevice->SetRenderState(D3DRS_STENCILFAIL, D3DSTENCILOP_ZERO);  // ステンシルフェイルをゼロに設定
...
//---------------------------------------------------
bool CRenderer::GetBackBufferToClientRatio(D3DXVECTOR2* ratio) const
{
	// DirectXのサイズを取得する
	D3DXVECTOR2 dxSize;
...
// レンダーターゲットサイズの取得
//----------------------------
bool CRenderer::GetTargetSize(D3DXVECTOR2* size) const
{
	// DirectXのサイズを取得する
...
//---------------------------------------------------
bool CRenderer::GetTargetToClientRatio(D3DXVECTOR2* ratio) const
{
	// DirectXのサイズを取得する
	D3DXVECTOR2 dxSize;
...
// ビューポートサイズの取得
//----------------------------
bool CRenderer::GetViewportSize(D3DXVECTOR2* size) const
{
	// DirectXのサイズを取得する
...
//---------------------------------------------------
bool CRenderer::GetViewportToClientRatio(D3DXVECTOR2* ratio) const
{
	// ビューポートのサイズを取得する
...
// スクリーンサイズ変更
//----------------------------
bool CRenderer::ReSize(const BOOL bWindow)
{
...
// カーソル位置の取得
//----------------------------
bool CRenderer::GetCursorPos(D3DXVECTOR2* pOutpos) const
{
	POINT cursorPint = CMain::GetCursorPoint();
...
// レンダーターゲットの変更
//----------------------------
bool CRenderer::ChangeTarget(RENDER_TARGET target)
{
	// レンダーターゲット
	switch (target)
	{
...
// 画面サイズの頂点バッファを生成
//----------------------------
bool CRenderer::CreateViewportVertexBuffer(LPDIRECT3DVERTEXBUFFER9* ppVtxBuffer, Color color) const
{
	if (ppVtxBuffer == nullptr) return E_POINTER;
	...
	array<DirectX2D::Vertex, VT_DEF> vertex;
	for (Index8 cntVtx = 0; cntVtx < VT_DEF; cntVtx++)
	{
	...
	SetVertex2D(ppVtxBuffer, vertex);
...
// リセット後の再生成
//----------------------------
bool CRenderer::ResetCreate(void)
{
...
// テクスチャをViewport全体に張る
//--------------------------------
bool CRenderer::RenderTexture(LPDIRECT3DTEXTURE9 pTexture, D3DCOLOR color, bool blendOne, bool zEnable, bool bAlphaTest, bool bPoint)
{
...
// リセット後の再生成
//----------------------------
bool CRenderer::SetTexture(void)
{
	// バックバッファのサイズ
...
// リセット後の再生成
//----------------------------
bool CRenderer::CreatePrint(void)
{
    // フォントの生成
    ...
    {
        return true; // フォントの生成成功
    }
    return false; // フォントの生成失敗
}

//----------------------------
// リセット後の再生成
//----------------------------
void CRenderer::ReleasePrint(void)
{
    // フォント破棄
    if (m_pFont != nullptr)
    {
...
// リセット後の再生成
//----------------------------
void CRenderer::DrawPrint(const std::string_view string)
{
    D3DXVECTOR2 screenSize{};
    if (FAILED(GetViewportSize(&screenSize))) return; // ビューポートのサイズ取得失敗
    RECT rect{ 0,0,LONG(screenSize.x),LONG(screenSize.y) };

    m_pFont->DrawText(nullptr, string.data(), -1, &rect, m_flag, D3DCOLOR_COLORVALUE(m_color.r, m_color.g, m_color.b, m_color.a));
