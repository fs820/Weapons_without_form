...
HRESULT CPlayer::Init(D3DXVECTOR3 pos, D3DXVECTOR3 rot, D3DXVECTOR3 scale, float fSpeed, float fRotSpeed)
{
	...
	SetAfterimage(true);
	m_pShadow = CShadow::Create(Transform(pos, CMath::VectorToEuler(CManager::GetLight()->GetLightVector(0)), scale), 2); // シャドウの生成
...
//更新処理
//------------------------------
void CPlayer::Update(const context::UpdateContext updateContext)
{
	...
	{// 死んでなければ
		...
		SetPosition(D3DXVECTOR3(pos.x, CManager::GetScene().Get3D().GetHeight(pos), pos.z)); // 移動
	...
	// 影の更新
	Transform transform = GetTransform();
	Vector3 lightRot = CMath::VectorToEuler(CManager::GetLight()->GetLightVector(0)); // ライトの方向をオイラー角に変換
	transform.rot = lightRot; // 影の回転をライトの方向の反対に合わせる
	m_pShadow->SetTransform(transform);
...
//描画処理
//------------------------------
void CPlayer::Draw(const context::DrawContext drawContext)
{
