...
D3DXVECTOR2 CRenderer::GetCursorPos(void) const
{
	...
	// クライアント座標とバックバッファサイズの比率
	D3DXVECTOR2 ratio{};
	if (FAILED(GetDxScreenToClientRatio(&ratio))) return D3DXVECTOR2(~0, ~0);
	...
	if (FAILED(GetDxScreenSize(&screenSize))) return D3DXVECTOR2(~0, ~0);
...
HRESULT CRenderer::GetCursorPos(D3DXVECTOR2* pOutpos) const
{
	...
	// クライアント座標とバックバッファサイズの比率
	D3DXVECTOR2 ratio{};
	if (FAILED(GetDxScreenToClientRatio(&ratio))) return E_FAIL;
	...
	// バックバッファサイズ
	D3DXVECTOR2 screenSize{};
	if (FAILED(GetDxScreenSize(&screenSize))) return E_FAIL;
